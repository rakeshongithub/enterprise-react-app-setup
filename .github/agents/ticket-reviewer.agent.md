...
handoffs:
  - label: Send findings to Delivery
    ...
    prompt: Prepare conservative delivery from the review findings. Do not commit, push, or release without separate explicit user approval.
---
...
A clean review means no known blocking findings, not that risk is zero. Always summarize the review before offering the Delivery handoff. The user approving the handoff button is the approval to begin delivery validation.
