...
handoffs:
  - label: Request skeptical review
    ...
    prompt: Before doing any review work, ask the user to explicitly confirm that the implementation is ready for review. Wait for confirmation. Then review the implementation against the ticket and plan. Be skeptical about regressions, accessibility, state ownership, tests, and scope. Do not edit unless explicitly asked.
---
...
If the plan is ambiguous or a material architecture choice changes, stop and ask the user. Before offering the Reviewer handoff, summarize the implementation and ask whether to proceed. Do not invoke the Reviewer automatically. Handoff must include files changed, tests, commands and results, coverage, and unresolved risks.
